  ...
  <MyCard class="monthly-tasks" :statusClass="monthActivity ? 'active' : ''">
    ...
    <template #default>
      ...
      <div class="action-row">
        <button class="action-button secondary" :disabled="monthLoading || fishToppingUp || arenaToppingUp"
...
const topUpMonthly = (type) => {
  ...
  return isFish ? autoTopUpFish(need, shouldBe, target) : autoTopUpArena(need, shouldBe, target)
}
...
const completeMonthly = (type) => {
  ...
  return isFish ? autoTopUpFish(need, target, target) : autoTopUpArena(need, target, target)
}

const onFishMoreSelect = (key) => { if (key === 'complete-fish') completeMonthly('fish') }
const onArenaMoreSelect = (key) => { if (key === 'complete-arena') completeMonthly('arena') }
...
const autoTopUpFish = async (need, shouldBe, target) => {
  ...
  try {
    ...
    if (!role) { try { await tokenStore.sendGetRoleInfo(tokenId) } catch {} role = tokenStore.gameData?.roleInfo?.role }
    let freeUsed = 0
    ...
    while (remaining > 0) {
      ...
      } catch (e) { message.error(`钓鱼失败：${e.message}`); break }
      remaining -= batch
      await new Promise(r => setTimeout(r, 800))
    }
    await fetchMonthlyActivity()
    if (fishNum.value >= shouldBe || fishNum.value >= target) message.success('钓鱼补齐完成')
...
const autoTopUpArena = async (need, shouldBe, target) => {
  ...
  try {
    const tokenId = tokenStore.selectedToken.id
    try { await tokenStore.sendMessageWithPromise(tokenId, 'arena_startarea', {}, 6000) } catch {}
    let safetyCounter = 0
    ...
    if (arenaNum.value >= shouldBe || arenaNum.value >= target) message.success('竞技场补齐完成')
    else if (safetyCounter >= safetyMaxFights) message.warning('达到安全上限，已停止竞技场补齐')
    else message.warning('竞技场补齐已停止，未达到目标')
  } finally { arenaToppingUp.value = false }
}
...
.monthly-row { display: flex; justify-content: space-between; margin-bottom: var(--spacing-xs); font-size: var(--font-size-sm); }
.description.muted { color: var(--text-tertiary); margin-top: var(--spacing-sm); }
.action-row { display: flex; gap: var(--spacing-sm); .action-button { flex: 1; } }
